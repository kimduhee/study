...
# 임베딩
임베딩은 텍스트, 이미지 같은 데이터를 숫자 벡터(vector)로 변환하는 기술이다.
즉, 데이터의 의미를 숫자로 표한하는 것이다. 컴퓨터는 문장의 의미를 직접 이해하지 못한다.
그렇기 때문에 문장을 숫자 벡터로 변환해서 의미를 비교할 수 있게 한다.
...
임베딩 
<pre><code>[0.12, -0.45, 0.91, 0.03, ...(768개)] 예) 768차원 일 경우
</code></pre>
...
+ 임베딩 모델이 생성
### 2)Sparse(희소벡터)
<pre><code>[0, 0, 5.2, 0, 0, 1.7, 0, 0, 0 ...]</code></pre>
+ 대부분 0
+ 특정 단어 위치만 값 존재
+ 키워드 기반
### 3)차이점
